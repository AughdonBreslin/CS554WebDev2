...
const updateRecipe = async function updateRecipe(id, updatedRecipe, userIdUpdating) {
    ...
    if(updatedRecipe.title) {
        ...
        validation.checkString(updatedRecipe.title, 1, 40, 'title', true, false, true);
    }
    ...
    await recipeCollection.updateOne({_id: new ObjectId(id)},{ $set: updatedRecipeData});
...
const createComment =  async function createComment(comment, recipeId, userThatPostedComment) {
    ...
    // Update recipe
    const recipeCollection = await recipes();
    let acknowledgement = await recipeCollection.updateOne({_id: new ObjectId(recipeId)}, {$set: {comments: recipe.comments}});
...
const removeRecipeComment = async function removeRecipeComment(recipeId, commentId, userIdDeleting) {
    ...
    let recipe = await recipeCollection.findOne({'_id': new ObjectId(recipeId)});
    ...
    recipe = await recipeCollection.findOne({'comments._id': new ObjectId(commentId)});
    ...
    const deletionInfo = await recipeCollection.findOneAndUpdate({'comments._id': new ObjectId(commentId)},
...
const toggleLike = async function toggleLike(recipeId, userId, removeLike) {
    ...
    let deletionInfo;
    if(removeLike) {
        deletionInfo = await recipeCollection.findOneAndUpdate({'_id': new ObjectId(recipeId)},
                                        {$pull:{likes: userId}});
    } else {
        deletionInfo = await recipeCollection.findOneAndUpdate({'_id': new ObjectId(recipeId)},
                                        {$push:{likes: userId}});
